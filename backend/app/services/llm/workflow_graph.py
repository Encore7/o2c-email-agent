...
def build_email_workflow_graph(service: LangChainLLMService):
    ...
    graph.add_node("classify_extract", lambda state: classify_extract_node(service, state))
    ...
    graph.add_edge(START, "classify_extract")
    graph.add_edge("classify_extract", "invoice_match")
